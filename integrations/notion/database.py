# integrations/notion/database.py
"""
Notion 데이터베이스 서비스
데이터베이스 항목 생성 및 관리
"""
...
class NotionDatabaseService(NotionBaseService):
    ...
    def __init__(self, log_init=True):
        """
        서비스 초기화
        
        Args:
            log_init: 초기화 로그 출력 여부
        """
        super().__init__(log_init=log_init)
    ...
    def _check_and_update_schema(self):
        ...
        try:
            ...
            logger.debug(f"현재 데이터베이스 프로퍼티: {list(current_props.keys())}")
            ...
            if missing_props:
                ...
                logger.debug("✅ 모든 필요한 프로퍼티가 존재합니다.")
