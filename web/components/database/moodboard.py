# web/components/database/moodboard.py
"""
...
def show_moodboard_modal(video_id: str):
    """무드보드 모달 표시 (사용되지 않음 - 인라인으로 대체)"""
...
def get_video_with_scenes(video_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # 분석 결과 가져오기
        analysis_result = db.get_latest_analysis(video_id)
        ...
        logger.error(f"Error getting video with scenes: {str(e)}")
        return None
